...
- [ ] **Step 7: Deploy a private candidate.** Only after all required evidence versions exist, deploy revision `hkbuddy-v1-api-<first-12-hex>` under private tag `candidate-<first-12-hex>` with exact origin `https://candidate-<first-12-hex>---hkbuddy-v1-api-582852715831.asia-east2.run.app`; attach the exact runtime identity, Cloud SQL/GCS/AI settings, numeric secret/evidence versions, one-instance cap, always-allocated CPU, dependency-safe startup plus separate liveness/readiness probes, and frozen image digest. Preserve the previous evidenced revision at 100% and assign the candidate 0% on later releases. On an exact empty-host first release, assign the sole candidate 100%, keep IAM private with no `allUsers`, run authenticated acceptance through the tag, and bind `private-bootstrap-100` plus no-prior state into its receipt.
...
- [ ] **Step 5: Promote once.** Freshly revalidate candidate service/revision/IAM/image plus immutable readiness/workload/mobile artifacts and 200 production traces. Only then add the reviewed public invoker, route `hkbuddy-v1-api` 100% to the evidenced revision, remove the candidate tag, and read back stable origin `https://hkbuddy-v1-api-582852715831.asia-east2.run.app`, traffic, revision, digest, min/max instances, IAM, SQL, bucket, alerts, and budget. If a first-release IAM/traffic/readback is ambiguous, restore and freshly verify private IAM, the tagged 100% candidate, revision, image, and evidence. If a later post-promotion gate fails, separately confirm rollback to the exact previous evidenced V1 revision at 100%; never delete data/resources or route to legacy.
