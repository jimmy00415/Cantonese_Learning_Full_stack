...
- [ ] **Step 5: Create the exact production resource island.** Create or exactly read back repository `hkbuddy-v1`, public stable service `hkbuddy-v1-api`, private candidate service `hkbuddy-v1-api-candidate`, instance/database `hkbuddy-v1-pg` / `hkbuddy_v1`, bucket `hkbuddy-v1-582852715831-media`, VPC/subnet/PSA range `hkbuddy-v1-vpc` / `hkbuddy-v1-ae2-run` / `hkbuddy-v1-google-services`, exact `hkbuddy-v1-*` service accounts, secrets, Jobs, monitoring policies, and budget. Generate session/database secrets in memory and add them as numeric Secret Manager versions without printing them.
...
- [ ] **Step 7: Deploy a private candidate.** Only after all required evidence versions exist, deploy revision `hkbuddy-v1-api-candidate-<first-12-hex>` to the separate `hkbuddy-v1-api-candidate` service at private 100% traffic under tag `candidate-<first-12-hex>` with exact target origin `https://candidate-<first-12-hex>---hkbuddy-v1-api-candidate-582852715831.asia-east2.run.app`; attach the exact runtime identity, Cloud SQL/GCS/AI settings, numeric secret/evidence versions, one-instance cap, always-allocated CPU, dependency-safe startup plus separate liveness/readiness probes, and frozen image digest. Mint the in-memory ID token for the untagged candidate-service root and keep exact candidate IAM limited to the reviewed private invoker with no public principal. Pin `run.googleapis.com/invoker-iam-disabled` to exact lowercase string `false` in both controlled Service specs; require raw Cloud Run v1 readbacks to prove IAM checking through annotation absence or an unpadded case-insensitive `false`, exact apiVersion/kind, and authoritative status traffic. Bind normalized `invokerIamDisabled=false` into the hashed candidate contract and require it across deploy/readback, promotion, cleanup, rollback, response-loss, and compensation; reject malformed/boolean/true/drifted values before dependent mutation. Bind `trafficState=candidate-service-private-100`, both service identities, and stable absent or genuine-prior-stable-at-100 state into the receipt. Do not mutate `hkbuddy-v1-api` during acceptance.
...
- [ ] **Step 1: Verify the candidate consumes the preboot evidence.** Read back the six numeric Secret Manager mounts on the frozen digest-pinned revision and require readiness to bind the same provider/config, dependency, artifact, and release-SHA digests. Do not repeat paid provider calls merely to make a failed gate green.
...
- [ ] **Step 5: Promote once.** Freshly revalidate the private candidate service/revision/IAM/image/config plus immutable readiness/workload/mobile artifacts and 200 production traces, including the enabled Invoker IAM check on every stable/candidate and compensation readback. On the first release, journal and delete the receipt-proven private candidate, prove canonical candidate absence, create `hkbuddy-v1-api` privately at 100%, verify service/revision/image/config and private IAM, then add `allUsers:roles/run.invoker` as the final mutation and perform only IAM readback afterward. Read back stable origin `https://hkbuddy-v1-api-582852715831.asia-east2.run.app`, traffic, revision, digest, min/max instances, IAM, SQL, bucket, alerts, and budget. Never automatically restore IAM after a lost public-grant response: adopt exact public readback, or fail closed on ambiguity without a second IAM mutation. Confirmed later `candidate` and `promote` phases fail before any cloud mutation with `ROLLING_RELEASE_UNSUPPORTED_SINGLETON` until release-lane dispatch and separate worker leadership make overlapping runtimes safe. Candidate cleanup remains receipt-bound: an exact already-absent precheck skips mutation and still repeats canonical absence; raw null, generic 404, wrong identity, or ambiguous transport fails closed. Rollback mutates only receipt-proven stable traffic. Never delete data/resources or route to legacy.
...
- [x] The final post-staging proof is an append-only attempt-bound checkpoint.
  ...
  bound to final intent. Pre-handoff expired paths re-proof safely. Every resumed
  first-release mutation revalidates authority and durable before-state; after
  candidate deletion, exact public IAM is adopted or exact private IAM retries
  only the journaled idempotent grant. Mixed/ambiguous paths block. No cloud
  mutation follows the terminal mutation.
