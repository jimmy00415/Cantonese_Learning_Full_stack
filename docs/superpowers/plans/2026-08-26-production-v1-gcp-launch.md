...
- [ ] **Step 5: Promote once.** Freshly revalidate the private candidate service/revision/IAM/image/config plus immutable readiness/workload/mobile artifacts and 200 production traces, including the enabled Invoker IAM check on every stable/candidate and compensation readback. On a later release, preserve the genuine prior stable revision at 100%, stage the accepted stable revision untagged at 0%, verify its exact image/config and absence of every stable tag, then atomically switch it to 100% while stable public IAM remains exact read-only state. On the first release, create `hkbuddy-v1-api` privately at 100%, verify service/revision/image/config and private IAM, then add `allUsers:roles/run.invoker` as the final mutation and perform only IAM readback afterward. Read back stable origin `https://hkbuddy-v1-api-582852715831.asia-east2.run.app`, traffic, revision, digest, min/max instances, IAM, SQL, bucket, alerts, and budget. Response-loss compensation restores the exact prior public stable state on later releases or the accepted private stable state and private IAM on the first release. Candidate cleanup separately validates its receipt, deletes only `hkbuddy-v1-api-candidate`, and verifies canonical absence; an exact already-absent initial precheck skips revision/artifact/IAM/delete but still repeats the candidate-specific canonical absence readback, while raw null/generic 404/ambiguous transport fails closed. Rollback mutates only receipt-proven stable traffic and does not depend on the candidate service. Never delete data/resources or route to legacy.
